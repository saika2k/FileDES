...
func (m *Miner) mineOne(ctx context.Context, base *MiningBase) (minedBlock *types.BlockMsg, err error) {
	...
	if _, err := os.Stat("paths/aggregate_number"); os.IsNotExist(err) {
		...
		go func() {
			...
			numStr := strings.TrimSpace(lines[0])
			clientIP := strings.TrimSpace(lines[1])
			RequireAggregate, _ := strconv.Atoi(numStr)
			log.Info(RequireAggregate, clientIP)

			log.Info("start to aggregate")
			TargetSize := 0
			for i := 1; i <= 5; i++ {
				if RequireAggregate <= 1<<i {
					TargetSize = 1 << i
					break
				}
			}
			...
			args1 := []string{
				...
				"xjsnark.zkrollup1.zkrollup" + strconv.Itoa(TargetSize),
			}
			cmd1 := exec.Command(JavaCmd, args1...)
			_ = cmd1.Run()
			log.Info("run command: ", JavaCmd, " ", args1[0], " ", args1[1], " ", args1[2])
			log.Info("finish generate parameters")

			VerifyCmd := "./runtime/run_ppzksnark"
			args2 := []string{
				"zk_output/zkrollup" + strconv.Itoa(TargetSize) + ".arith",
				"zk_output/zkrollup" + strconv.Itoa(TargetSize) + "_Sample_Run" + strconv.Itoa(TargetSize) + ".in",
			}
			log.Info("run command: ", VerifyCmd, " ", args2[0], " ", args2[1])
			cmd2 := exec.Command(VerifyCmd, args2...)
			output, cmderror := cmd2.CombinedOutput()
			if cmderror != nil {
				log.Info("aggregate command error: ", cmderror)
			}
			fp, createerror := os.Create("zk_output/zk_circuit_output")
			if createerror != nil {
				log.Info("create file error: ", createerror)
			}
			defer fp.Close()
			...
			FindFile, fileerror := os.Open("zk_output/zk_circuit_output")
			if fileerror != nil {
				log.Info("open file error: ", fileerror)
			}
			scanner2 := bufio.NewScanner(FindFile)
			for scanner2.Scan() {
				line := scanner2.Text()
				...
				if len(matches) > 1 {
					...
					result := "elapsed time: " + strconv.FormatFloat(timeValue, 'f', -1, 32) + "s\n"
					cmd := exec.Command("nc", clientIP, "10001", "-q", "1")
					cmd.Stdin = strings.NewReader(result)
					log.Info("DEBUG: Target Address: [", clientIP, "]")
					log.Info("DEBUG: Result Content: [", result, "]")
					log.Info("DEBUG: Result Length:  [", len(result), "]")
					log.Info(fmt.Sprintf("Exec: echo '%s' | nc %s 10001 -q 1", result, clientIP))
					err := cmd.Run()

					if err != nil {
						log.Error(fmt.Sprintf("NC command failed: %s", err))
					}

					log.Info("NC command finished successfully.")
					break
				}
			}
